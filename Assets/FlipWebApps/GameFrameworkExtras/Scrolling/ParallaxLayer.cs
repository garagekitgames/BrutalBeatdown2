...
namespace GameFramework.Scrolling.Components
{
    ...
    public class ParallaxLayer : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            // For some reason teh base constructor isn't called (not sure how that is possible)!!!
            // We recreate the objects here to make sure they are setup correctly.
            for (int i = 0; i < SpawnableItems.Length; i++)
            {
                var displayItem = SpawnableItems[i];
                ...
                SpawnableItems[i] = displayItemNew;
            }
        ...
        void Update()
        {
            ...
            if (movementDirection == MovementDirectionType.Left)
            {
                ...
                if (!_isManualSetupMode && firstElement.GetRightEdge() < AreaBounds.Min)
                {
                    ...
                    firstElement.Pool.Despawn(firstElement);
                ...
                if (_areaEdges.Max <= AreaBounds.Max)
                {
                    if (_isManualSetupMode)
                    {
                        ...
                        firstElement.OnDespawn();
                        firstElement.OnSpawn();
            ...
            else if (movementDirection == MovementDirectionType.Right)
            {
                ...
                if (!_isManualSetupMode && lastElement.GetLeftEdge() > AreaBounds.Max)
                {
                    ...
                    lastElement.Pool.Despawn(lastElement);
        ...
        void AddDisplayItemInstance(LocationType location)
        {
            ...
            var loopingElement = spawnableItem.SpawnPoolItem(transform);//.position + spawnableItem.Prefab.transform.position, 
        ...
        public class SpawnableItem : PoolGeneric<DisplayItemInstance>
        {
            ...
            base(prefab, preInitialiseCount, SizeExceededModeType.IncreasePoolSize, maxCapacity == 0, Mathf.Max(preInitialiseCount, maxCapacity), MaximumSizeExceededModeType.ReturnNull, inactiveParent)
            {
            }
            //Transform _parent;
